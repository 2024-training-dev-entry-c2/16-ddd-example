...
class PlayerTest {
	...
	void addSurvivorSuccess() {
		...
		assertEquals("Mariana", player.getName());
	...
	void unlockSkillSuccess() {
		...
		assertEquals(1, player.getSurvivors().size());
		assertEquals(2, player.getUncommittedEvents().size());
		assertInstanceOf(UnlockedSkill.class, player.getUncommittedEvents().get(1));
	}
	...
	void chooseSkillSuccess() {
		player.addSurvivor("1", "Mariana");
		player.chooseSkill("10", "1");
		assertEquals(1, player.getSurvivors().size());
		assertEquals("Sigilo", player.getSurvivors().get(0).getSkills().get(0).getName().getValue());
		assertEquals(2, player.getUncommittedEvents().size());
